...
import static dev.amoeba.moexplorer.misc.IntentUtils.ACTION_OPEN_DOCUMENT_TREE;
import static dev.amoeba.moexplorer.provider.ExternalStorageProvider.ROOT_ID_SECONDARY;
...
public class SAFManager {
    ...
    public static String getRootUri(Uri uri) {
        if (isTreeUri(uri)) {
            return dev.amoeba.moexplorer.model.DocumentsContract.getTreeDocumentId(uri);
        }
        return dev.amoeba.moexplorer.model.DocumentsContract.getDocumentId(uri);
    }

    private static Uri buildDocumentUriMaybeUsingTree(Uri uri, String docId) {
        return dev.amoeba.moexplorer.model.DocumentsContract.buildDocumentUriMaybeUsingTree(uri, docId);
    }

    private static boolean isTreeUri(Uri uri) {
        return dev.amoeba.moexplorer.model.DocumentsContract.isTreeUri(uri);
    }
