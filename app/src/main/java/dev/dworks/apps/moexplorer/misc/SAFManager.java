...
import static dev.dworks.apps.moexplorer.misc.IntentUtils.ACTION_OPEN_DOCUMENT_TREE;
import static dev.dworks.apps.moexplorer.provider.ExternalStorageProvider.ROOT_ID_SECONDARY;
...
public class SAFManager {
    ...
    public static String getRootUri(Uri uri) {
        if (isTreeUri(uri)) {
            return dev.dworks.apps.moexplorer.model.DocumentsContract.getTreeDocumentId(uri);
        }
        return dev.dworks.apps.moexplorer.model.DocumentsContract.getDocumentId(uri);
    }

    private static Uri buildDocumentUriMaybeUsingTree(Uri uri, String docId) {
        return dev.dworks.apps.moexplorer.model.DocumentsContract.buildDocumentUriMaybeUsingTree(uri, docId);
    }

    private static boolean isTreeUri(Uri uri) {
        return dev.dworks.apps.moexplorer.model.DocumentsContract.isTreeUri(uri);
    }
